...
import {
    addresses,
    decimals,
    spacefiRouterContractAddress,
    zkswapRouterContractAddress
...
import {zkswapRouterAbi} from "../data/abi/zkswap_router";

export const Zkswap = async (privateKey: Hex) => {
    const logger = makeLogger('Zkswap  ')
    ...
    const zkswapRouterContract = getContract({
        address: zkswapRouterContractAddress,
        abi: zkswapRouterAbi,
        client: zksyncWallet
    })

    const getMinAmountOut = async (amountIn: bigint, path: Hex[]) => {
        const response = await zkswapRouterContract.read.getAmountsOut([
            amountIn,
            path
        ]) as [bigint, bigint]
    ...
    const swapEthToToken = async (uintValue: bigint, toToken: tokens = 'USDC') => {
        ...
        while (!isSuccess) {
            try {
                ...
                const txHash = await zkswapRouterContract.write.swapExactETHForTokens([
                    minAmountOut,
                    swapPath,
                    walletAddress,
                    deadline,
    ...
    const swapTokenToEth = async (fromToken: tokens = 'USDC') => {
        ...
        while (!isSuccess) {
            try {
                ...
                const txHash = await zkswapRouterContract.write.swapExactTokensForETH([
                    uintValue,
                    minAmountOut,
                    swapPath,
                    walletAddress,
